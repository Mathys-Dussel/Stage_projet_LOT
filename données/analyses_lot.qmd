...
ps_top <- prune_taxa(top_otus, ps)
    ...
    tax_num <- apply(tax_df, 2, function(x) as.numeric(as.factor(x)))
    rownames(tax_num) <- taxa_names(ps_top)

    arbre_taxo <- as.phylo(hclust(dist(tax_num)))
...
tax <- as.data.frame(tax_table(ps_top))
phylums <- as.factor(tax$gbr268_Phylum)
couleurs <- rainbow(length(levels(phylums)))[phylums]

par(mar=c(1,1,1,15)) 
plot(phy_tree(ps_top), 
...
phy_tree(ps_top)$tip.label <- tax$gbr268_Family
...
legend("topleft", legend = levels(phylums), fill = rainbow(length(levels(phylums))), cex = 0.8)
...
ggtree(ps_top, aes(color = gbr268_Phylum), ladderize = TRUE) +
  geom_tiplab(aes(label = gbr268_Family), size = 3, offset = 0.01) +
