...
nmds_bray <- ordinate(ps_hellinger, method = "NMDS", distance = "bray", k = 3)

print(paste("Valeur de stress :", round(nmds_bray$stress, 3)))
stressplot(nmds_bray, main = "Shepard plot")

library(goeveg)
dimcheckMDS(as(otu_table(ps_hellinger), "matrix"), distance = "bray", k = 6, trymax = 20)
...
modele_lm <- lm(nmds_bray$points[, 1] ~ organ + plant_family+project, data = metadata_global)
summary(modele_lm)
...
```{r}
# | label: test-permanova-globales

library(vegan)

adonis2(
  bray_dist ~ project * organ + plant_family, 
  data = metadata_global, 
  permutations = 999
)

# pour tester l'effet de project en contrôlant pour organ et plant_family
...
# on fait margin pour tester l'effet de chaque facteur en contrôlant pour les autres, ce qui est plus pertinent dans ce cas où les facteurs sont potentiellement confondus (par exemple, certains organes peuvent être sur-représentés dans certains projets).
...
```{r}
#| label: ordination-LOT-1

ps_lot1 <- subset_samples(ps_hellinger, project == "LOT1")
nmds_lot1 <- ordinate(ps_lot1, method = "NMDS", distance = "bray")

stressplot(nmds_lot1, main = "Shepard plot")

bray_dist_lot1 <- vegdist(as(otu_table(ps_lot1), "matrix"), method = "bray")
meta_lot1 <- as(sample_data(ps_lot1), "data.frame")




plot_ordination(ps_lot1, nmds_lot1, color = "organ") +
  geom_point(aes(shape = position), size = 3, alpha = 0.8) +
  stat_ellipse(aes(group = organ)) +
  theme_bw() +
  labs(title = "NMDS - Projet LOT1 (Bray-Curtis, Hellinger) avec ellipses de confiance")



adonis2(bray_dist_lot1 ~ organ, data = meta_lot1, permutations = 999)





plot_ordination(ps_lot1, nmds_lot1, color = "plant_family") +
  geom_point(aes(shape = organ), size = 3, alpha = 0.8) +
  stat_ellipse(aes(group = plant_family)) +
  theme_bw() +
  labs(title = "NMDS - Projet LOT1 (Hellinger) par Famille de plante avec ellipses")

adonis2(bray_dist_lot1 ~ plant_family, data = meta_lot1, permutations = 999)


```

```{r}
#| label: ordination-LOT-2

ps_lot2 <- subset_samples(ps_hellinger, project == "LOT2")
nmds_lot2 <- ordinate(ps_lot2, method = "NMDS", distance = "bray")

stressplot(nmds_lot2, main = "Shepard plot")

bray_dist_lot2 <- vegdist(as(otu_table(ps_lot2), "matrix"), method = "bray")
meta_lot2 <- as(sample_data(ps_lot2), "data.frame")

plot_ordination(ps_lot2, nmds_lot2, color = "organ") +
    geom_point(aes(shape = position), size = 3, alpha = 0.8) +
    stat_ellipse(aes(group = organ)) +
    theme_bw() +
...
adonis2(bray_dist_lot2 ~ organ, data = meta_lot2, permutations = 999)



plot_ordination(ps_lot2, nmds_lot2, color = "plant_family") +
    geom_point(aes(shape = organ), size = 3, alpha = 0.8) +
    stat_ellipse(aes(group = plant_family)) +
    theme_bw() +
    labs(title = "NMDS - Projet LOT2 par Famille de plante avec ellipses")

adonis2(bray_dist_lot2 ~ plant_family, data = meta_lot2, permutations = 999)
```


```{r}
#| label: ordination-LOT-3

ps_lot3 <- subset_samples(ps_hellinger, project == "LOT3")
nmds_lot3 <- ordinate(ps_lot3, method = "NMDS", distance = "bray")
bray_dist_lot3 <- vegdist(as(otu_table(ps_lot3), "matrix"), method = "bray")
meta_lot3 <- as(sample_data(ps_lot3), "data.frame")

plot_ordination(ps_lot3, nmds_lot3, color = "organ") +
    geom_point(aes(shape = position), size = 3, alpha = 0.8) +
    stat_ellipse(aes(group = organ)) +
    theme_bw() +
    labs(title = "NMDS - Projet LOT3 (Bray-Curtis) avec ellipses de confiance") 

adonis2(bray_dist_lot3 ~ organ, data = meta_lot3, permutations = 999)

plot_ordination(ps_lot3, nmds_lot3, color = "plant_family") +
    geom_point(aes(shape = organ), size = 3, alpha = 0.8) +
    stat_ellipse(aes(group = plant_family)) +
    theme_bw() +
    labs(title = "NMDS - Projet LOT3 par Famille de plante avec ellipses")

adonis2(bray_dist_lot3 ~ plant_family, data = meta_lot3, permutations = 999)
```
...
# # library(phyloseq)
# 
# ps_phylum <- tax_glom(ps, taxrank = "gbr268_Phylum", NArm = FALSE)
# 
# otu_mat <- as(otu_table(ps_phylum), "matrix")
...
#                   endpoint = 2000)   
# 
# ggiNEXT(out_inext, type = 1, facet.var = "None") +
#   scale_shape_manual(values = rep(16, 100)) +
#   theme_bw() +
#   labs(title = "Courbes de raréfaction et d'extrapolation par phylum (q=0)",
...
#   library(dplyr)
# 
#   # Extraire la table des OTU et la table taxonomique
...
# 
#   # Combiner et agréger les données pour obtenir une matrice [OTUs x Phylums]
#   df <- as.data.frame(otu_mat)
#   df$Phylum <- tax_mat[rownames(df), "gbr268_Phylum"]
# 
#   otu_phylum_mat <- df |>
#     pivot_longer(cols = -Phylum, names_to = "Sample", values_to = "Abundance") |>
#     group_by(Phylum, OTU = rep(rownames(df), times = ncol(otu_mat))) |>
#     summarise(Abundance = sum(Abundance), .groups = "drop") |>
#     pivot_wider(names_from = Phylum, values_from = Abundance, values_fill = 0) |>
#     tibble::column_to_rownames("OTU") |>
#     as.matrix()
#     saveRDS(otu_phylum_mat, "otu_phylum_mat.rds")
# write.table(otu_phylum_mat, file = "otu_phylum_mat.txt", sep = "\t", quote = FALSE, col.names = NA)
# out_inext <- iNEXT(otu_phylum_mat, 
#                     q = 0,     
#                     datatype = "abundance", 
#                     nboot = 0,       
#                     endpoint = 4000)   
#   
# saveRDS(out_inext, "out_inext.rds")
# 
#   ggiNEXT(out_inext, type = 1, facet.var = "None") +
#     theme_bw() +
#     labs(title = "Courbes de raréfaction en OTUs par phylum (q=0)",
#          x = "Nombre total de séquences",
#          y = "Diversité en OTUs")
```
...
# Extraction et nettoyage des données de phyloseq
sam_df <- data.frame(sample_data(ps))
sam_df$Sample <- rownames(sam_df)

otu_df <- data.frame(otu_table(ps))
if (!taxa_are_rows(ps)) otu_df <- data.frame(t(otu_table(ps)))
otu_df$OTU <- rownames(otu_df)

tax_df <- data.frame(tax_table(ps))
tax_df$OTU <- rownames(tax_df)

# Trouver la colonne des espèces
...
# Mettre en forme longue les abondances
otu_long <- otu_df %>%
  pivot_longer(col = -OTU, names_to = "Sample", values_to = "Abundance") %>%
  filter(Abundance > 0)

# Combiner abondances, métadonnées, taxonomie et traits
...
# Fonction pour tracer les proportions de mode de vie
...
# Fonction simplifiée pour un test du Chi-2
...
# Génération des résultats si les colonnes sont trouvées
if (!is.na(col_lifestyle)) {
  ...
  # Heatmap des différences entre familles de plantes
