...
```{r}
  ...
  ps_class <- tax_glom(ps, taxrank = "gbr268_Class", NArm = FALSE)
  otu_vegan <- as(otu_table(ps_class), "matrix")
  if (taxa_are_rows(ps_class)) {
    otu_vegan <- t(otu_vegan)
  }

  rarecurve(otu_vegan, step = 100, label = FALSE, col = "steelblue",
            main = "Courbes de raréfaction par classe (vegan)",
            xlab = "Nombre de séquences", 
            ylab = "Classes observées")
```
