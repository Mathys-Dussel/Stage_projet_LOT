...
Enfin, je remercie tout particulièrement ma copine et ma mère pour leur soutien et pour avoir pris le temps de relire mon travail.
\newpage
...
Cette étude explore la composition et la structuration des communautés fongiques associées aux plantes épiphytes vasculaires dans trois sites des Andes tropicales (Pérou et Colombie), s'échelonnant de 400 à 2400 mètres d'altitude. En utilisant une approche de métabarcoding (gène ITS2), nous avons caractérisé les mycobiomes de 15 familles de plantes, en distinguant les organes (racines vs feuilles) et la position au sein des tissus (épiphytes vs endophytes).  
...
\begin{figure}[H]
    ...
    \caption{Analyse des intersections d'OTUs entre les différents compartiments et sites (diagrammes UpSet).}
    \label{fig:upset}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Ordination NMDS des communautés fongiques selon les sites et les organes.}
    \label{fig:nmds}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}[c]{0.33\textwidth}
        \caption[Tableau des OTUs les plus abondants dans chaque microhabitat.]{Tableau des OTUs les plus abondants dans chaque microhabitat. 
        \scriptsize
        Le dendrogramme représente le regroupement hiérarchique des Ordres selon leurs profils d'abondance relative. }
    ...
    \begin{figure}[H]
        ...
        \begin{minipage}[c]{0.25\textwidth}
            \caption{\footnotesize Composition taxonomique des communautés fongiques associées aux différentes familles de plantes tropicales.}
...
\section{Discussion}
Cette étude révèle une structuration marquée des communautés fongiques associées aux plantes épiphytes vasculaires tropicales. Grâce à un échantillonnage multi-sites et multi-microhabitats dans les Andes péruviennes, il apparaît que la diversité et la composition des assemblages fongiques sont avant tout déterminées par le microhabitat (organe et position épiphyte/endophyte). Le site géographique intervient ensuite, tandis que l’identité de la plante hôte, bien que significative, joue un rôle plus secondaire.

Le site d’échantillonnage s’impose comme un facteur structurant important. Les communautés fongiques diffèrent nettement entre les trois sites étudiés (LOT1, LOT2, LOT3). Cela reflète probablement l’influence de gradients environnementaux tels que l’altitude, le climat ou la composition floristique locale. Cependant, l’absence de mesures environnementales précises (température, humidité, lumière, etc.) limite la possibilité de relier directement ces différences à des facteurs abiotiques spécifiques.

Les organes de la plante (racines, feuilles jeunes, feuilles vieilles) constituent le facteur le  plus différenciant. Les racines hébergent une diversité alpha particulièrement élevée et de nombreux OTUs exclusifs, ce qui traduit une spécialisation écologique marquée. À l’inverse, les feuilles, qu’elles soient jeunes ou vieilles, présentent des communautés plus homogènes et partagent une partie de leur cortège fongique. Elles restent toutefois dominées par des groupes typiques des tissus aériens. La position (épiphyte/endophyte) module aussi la composition : chaque compartiment héberge des assemblages distincts, même si cet effet reste moins marqué que celui de l’organe ou du site.

L’identité de la plante hôte explique une part significative, mais plus modeste, de la variance dans la composition des communautés fongiques. Ce résultat suggère que l’identité phylogénétique de l’hôte agit comme un filtre supplémentaire. Toutefois, les contraintes du microhabitat et du site semblent primer. Il aurait été intéressant de disposer de mesures sur les traits fonctionnels des plantes (stratégies d’acquisition de ressources, morphologie, exsudats, etc.), car ces caractéristiques pourraient expliquer plus finement la structuration des communautés fongiques que la seule appartenance à une famille ou un genre. L’absence de ces données limite la compréhension des mécanismes de filtrage écologique à l’œuvre.
...
Plusieurs limites doivent être soulignées. L’échantillonnage n’a pas été spécifiquement conçu pour l’étude des communautés fongiques, ce qui peut introduire des biais de représentativité. L’approche métabarcoding basée sur l’ITS ne permet pas d’assigner précisément les fonctions écologiques des OTUs détectés (symbiotes, pathogènes, saprotrophes). Enfin, l’absence de données fonctionnelles sur les plantes hôtes et de mesures environnementales fines restreint l’interprétation des patrons observés. L’usage de bases de fonées fonctionnelles (Fungal Traits) s’est avérée infructueuse du à un faible nombre de correspondance (334 OTUs correspondent à ceux de Fungal Traits sur 1027 OTUs)

En somme, cette étude met en lumière la complexité et la structuration fine des communautés fongiques associées aux plantes épiphytes tropicales. Elle souligne le rôle central du microhabitat et des gradients environnementaux dans la diversification fongique. Ces résultats ouvrent la voie à des recherches futures qui pourraient intégrer davantage de dimensions fonctionnelles et écologiques : traits des hôtes, interactions biotiques, dynamique temporelle. L’exploration de la spécialisation des niches et des réseaux d’interactions au sein de ces systèmes tropicaux reste un champ prometteur.
